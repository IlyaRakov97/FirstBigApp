...
class TasksTableVIewCell: UITableViewCell {
    ...
    let taskDescription = UILabel(text: "Научиться писать extansion и правильно их применять", font: .avenirNext14())
    
    let readyButton: UIButton = {
        let button = UIButton()
        button.setBackgroundImage(UIImage(systemName: "space"), for: .normal)
    ...
    weak var cellTaskDeligate: PressReadyTaskButtonProtocol?
    var index: IndexPath?
    ...
    @objc func readyButtonTapped(){
        ...
        cellTaskDeligate?.readyButtonTapped(indexPath: index)
    }
    ...
    func setConstraints(){
        ...
        NSLayoutConstraint.activate([
            taskDescription.topAnchor.constraint(equalTo: topAnchor, constant: 5),
